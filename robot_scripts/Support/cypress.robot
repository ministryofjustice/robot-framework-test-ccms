...
Cypress runner
    ...
    ${result} =    Run Process  ${cypress_executable_path} run --config-file ${cypress_config_file} --spec ${cypress_script_path} ${cypress_options} --browser ${APPLY_BROWSER}  cwd=${cwd}  stderr=STDOUT  stdout=cypress_log.txt  shell=${shell_option}  output_encoding=UTF-8
    [return]    ${result}
